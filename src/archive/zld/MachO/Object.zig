...
pub fn parse(self: *Object, allocator: Allocator, cpu_arch: std.Target.Cpu.Arch) !void {
    ...
    @memset(self.source_section_index_lookup, -1);
    ...
    sort.heap(SymbolAtIndex, sorted_all_syms.items, self, SymbolAtIndex.lessThan);
    ...
    for (sorted_all_syms.items, 0..) |sym_id, i| {
        ...
        if (sym.sect() and self.source_section_index_lookup[sym.n_sect - 1] == -1) {
            self.source_section_index_lookup[sym.n_sect - 1] = @as(i64, @intCast(i));
        }
    ...
    if (self.hasUnwindRecords()) {
        ...
        @memset(self.unwind_relocs_lookup, .{
            .dead = true,
            .reloc = .{
                .start = 0,
                .len = 0,
...
pub fn getSymbolByAddress(self: Object, addr: u64, sect_hint: ?u8) u32 {
    ...
    if (sect_hint) |sect_id| {
        if (self.source_section_index_lookup[sect_id] > -1) {
            const first_sym_index = @as(usize, @intCast(self.source_section_index_lookup[sect_id]));
            ...
            if (target_sym_index > 0) {
                return @as(u32, @intCast(first_sym_index + target_sym_index - 1));
            }
        }
